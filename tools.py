...
def search_repo(
    repo_name: str, keywords: List[str], max_results: int = 20
    ...
    try:
        ...
        for keyword in keywords:
            ...
            for enhanced_keyword in enhanced_keywords:
                enhanced_results = _search_with_fts(
                    cursor, repo_name, [enhanced_keyword], max_results
                )
        ...
        if not search_results:
            logging.info(
                f"No results found for search: repo_name={repo_name}, keywords={keywords}"
            )
            return None

        logging.info(
            f"Search completed. Returning {len(search_results)} SearchResult objects"
        )
        ...
        logging.error(
            "Make sure you've run generate_database() from local_db.py first to create the database and FTS tables."
        )
        return []
...
def _search_with_fts(
    cursor, repo_name: str, keywords: List[str], max_results: int
) -> List[SearchResult]:
...
def read_repo_function(
    repo_name: str, func_path: str, func_name: str
) -> Optional[Function]:
    """
    ...
    try:
        ...
        logging.info(
            f"Reading function: repo_name={repo_name}, func_path={func_path}, func_name={func_name}"
        )
        ...
        if result is None:
            logging.warning(
                f"No function found for repo_name={repo_name}, func_path={func_path}, func_name={func_name}"
            )
            return None
        ...
        (
            ...
            db_func_path,
            db_func_name,
            db_whole_func_string,
            db_language,
            db_func_documentation_string,
            db_func_code_tokens,
        ) = result
...
if __name__ == "__main__":
    results = search_repo(
        repo_name="deepmind/sonnet", keywords=["curriculum"], max_results=5
    )
    print(results)
    print(
        read_repo_function(
            results[0].repo_name, results[0].func_path, results[0].func_name
        )
    )
